...
export const QuestionList = ({ searchQuery, topicId }: QuestionListProps) => {
    ...
    const { questions, pagination, isLoading, isError, mutate } =
        useQuestions(activePage, 20, searchQuery, topicId);
                    ...
                    <Card key={q.id} shadow="sm" padding="md" radius="md" withBorder>
                        <Grid gutter="lg">
                            ...
                            <Grid.Col span={{ base: 12, md: q.imageUrl ? 8 : 12 }}>
                                <Stack gap="xs">
                                    <Group justify="space-between" align="flex-start">
                                        ...
                                        <Group gap={5}>
                                            <Tooltip label={t("questions.viewTooltip")}>
                                                <ActionIcon variant="light" color="blue" onClick={() => handleView(q)}>
                                                    <IconEye size={16} />
                                                </ActionIcon>
                                            </Tooltip>
                                            <Tooltip label={t("questions.editTooltip")}>
                                                <ActionIcon variant="light" color="orange" onClick={() => handleEdit(q)}>
                                                    <IconEdit size={16} />
                                                </ActionIcon>
                                            </Tooltip>
                                            <Tooltip label={t("questions.deleteTooltip")}>
                                                <ActionIcon variant="light" color="red" onClick={() => handleDeleteClick(q)}>
                                                    <IconTrash size={16} />
                                                </ActionIcon>
                                            </Tooltip>
                                        </Group>
                                    ...
                                    <Group mt="md" gap="xl">
                                        {q.difficulty && (
                                            <Badge color={difficultyColors[q.difficulty]} variant="dot">
                                                {q.difficulty}
                                            </Badge>
                                        )}
                        ...
                        <Grid gutter="xs">
                            {q.options && q.options.length > 0 ? (
                                ...
                                <Grid.Col span={12}>
                                    <Text size="xs" c="dimmed">{t("common.noData")}</Text>
                                </Grid.Col>
                            )}
                        </Grid>
                    </Card>
